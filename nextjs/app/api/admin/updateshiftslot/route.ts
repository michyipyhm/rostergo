...
export async function POST(req: NextRequest) {
  ...
  try {
    const { id, title, short_title, start_time, end_time } = await req.json()

    const token = req.headers.get('Authorization')?.split(' ')[1]
    const payload = jwtDecode<Payload>(token)
    const branch_id = payload.branch_id

    await shiftSlotService.updateShiftSlot(id, branch_id, title, short_title, start_time, end_time)

    return NextResponse.json(
      { success: true, message: "Shift Slot updating successfully" },
      { status: 200 }
    );
  } catch (error) {
