...
function MonthlyRosterForm({ data }: { data: MonthlyRosterData }) {
  ...
  const [hoveredShiftData, setHoveredShiftData] =
  ...
  const handleCellMouseEnter = (
    ...
    const leaveRequestConfirmed =
      leaveRequestsConfirmed[memberId]?.[day - 1] || "";
    const leaveRequestPending = leaveRequestsPending[memberId]?.[day - 1] || "";

    //preview every staff' day
    const hoveredDetails = (
      <div>
        Staff: <strong>{member?.name}</strong>
        <br />
        <div>
          Shift: <strong>{shift || ""}</strong>
        </div>
        <div>
          Leave Confirmed: <strong>{leaveRequestConfirmed || ""}</strong>
        </div>
        <div>
          Shift Requests: <strong>{shiftRequestPending || ""}</strong>
        </div>
        <div>
          Leave Requests: <strong>{leaveRequestPending || ""}</strong>
        </div>
      </div>
    );
    setHoveredDay(day);
  ...
  useEffect(() => {
    if (tooltipRef.current) {
      ...
      setTooltipStyle({
        left: Math.max(mousePosition.x - tooltipWidth - 10, 10) + "px",
        top: Math.max(mousePosition.y - (tooltipHeight * 1) / 10, 10) + "px",
      });
    }
  }, [mousePosition]);
            ...
            <tr>
              ...
              {weekdays.map((weekday, i) => (
                <th
                  key={`weekday-${i}`}
                  className={`${
                    weekday === "Sat" || weekday === "Sun" ? styles.weekend : ""
                  } ${publicHolidays.has(i + 1) ? styles.holiday : ""}`}
                >
                  {weekday}
                </th>
              ))}
            ...
            {staff.map((member, index) => (
              <tr key={index}>
                ...
                {Array.from({ length: daysInMonth }, (_, i) => {
                  ...
                  const shiftRequestConfirmed =
                    shiftRequestsConfirmed[member.id]?.[i] || "";
                  const shiftRequestPending =
                    shiftRequestsPending[member.id]?.[i] || "";
                  const leaveRequestConfirmed =
                    leaveRequestsConfirmed[member.id]?.[i] || "";
                  const leaveRequestPending =
                    leaveRequestsPending[member.id]?.[i] || "";
                  const showTheDay =
                    leaveRequestConfirmed ||
                    leaveRequestPending ||
                    shiftRequestConfirmed ||
                    shift ||
                    shiftRequestPending;
                  const cellClassName = `
                  ...
                  return (
                    <td
                      key={i + 1}
                      onClick={() =>
                        handleCellClick(i + 1, member.id, member.name)
                      }
                      className={cellClassName}
                      onMouseEnter={(e) =>
                        handleCellMouseEnter(i + 1, member.id, e)
                      }
                      onMouseLeave={handleCellMouseLeave}
                    >
                      {showTheDay}
